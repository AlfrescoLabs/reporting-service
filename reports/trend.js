...
router.get('/api/alfresco/:version/trend/:day/:month/:year', processTrend);
router.get('/api/alfresco/:version/trend', processTrend);
/**
 * Get open bug list and populate db.
 */
function processTrend(req, res) {
        ...
        if(typeof issues !== 'undefined'){
          issues.map(function(issue) {
            ...
            if(issue.fields.labels.length > 0){
              //check if triaged
              var triaged = issue.fields.labels.filter(function(x){ return x === 'triaged'})
              if(triaged.length > 0){
